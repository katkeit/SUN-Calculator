...
public class Updater
{
    public static void main(String[] args)
    {
        ...
        fileIO.readFile("update/config/settings.txt");
        fileIO.readFile("update/config/preferences.txt");
        fileIO.readFile("update/download/version.txt");
        ...
        if (!FileIO.getAppVersion().matches(FileIO.getUpdateVersion()))
        {
            ...
            int input = JOptionPane.showOptionDialog(null, (String)Language.getUpdaterMessages().get(2) + FileIO.getUpdateVersion() + " " + (String)Language.getUpdaterMessages().get(3) + "\n" + (String)Language.getUpdaterMessages().get(4) + FileIO.getUpdatePage() + ").", "", -1, 1, null, (Object[])options, options[0]);
        ...
        if (downloader.startDownload(FileIO.getDownloadLink(), FileIO.getUpdateZip()))
        {
            ...
            {
                try {
                    Runtime.getRuntime().exec("java -jar " + FileIO.getCalcPath() + "SUN Calculator.jar");
                }
                catch (IOException e) {
                    e.printStackTrace();
                }
